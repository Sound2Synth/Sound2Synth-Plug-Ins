...
DexedAudioProcessor::DexedAudioProcessor() {
    ...
    clipboardContent = -1;
    // Modified from Dexed
    for (auto i = 0; i < getNumParameters(); i ++) {
        parameterNameToIndex.set(getParameterName(i), i);
    }
    

//    int i = parameterNameToIndex["OP4 R SCALE DEPTH"];
//    setParameter(i, 0.0);
}
...
void DexedAudioProcessor::matchFile()
{
    FileChooser matcher { "Please select a WAV" };
    if (matcher.browseForFileToOpen())
    {
        File file (matcher.getResult());
        
        URL url = URL(serverURL).withFileToUpload("Upload File", file, "wav");
        
        MemoryBlock block;
        if (url.readEntireBinaryStream (block, true)) {
            
            paramString = block.toString();
            
            json json_params = json::parse(paramString.toStdString());

            juce::StringArray paramsKey {};
            juce::Array<float> paramsValue {};

            for (json::iterator it = json_params.begin(); it != json_params.end(); ++it) {
                paramsKey.add(it.key());
                paramsValue.add(it.value());
            }
            for (int i = 0; i < paramsKey.size(); i++)
            {
                std::cout << paramsKey[i] << " : " << paramsValue[i] << std::endl;
            }
            
            for ( auto i = 0; i < paramsKey.size(); i++)
            {
                int paramIdx = parameterNameToIndex[paramsKey[i]];
                setParameter(paramIdx, paramsValue[i]);
            }
...
void DexedAudioProcessor::downloadFile()
{
    FileChooser download("Please select a location", dexedCartDir, "*.json");
    if ( download.browseForFileToSave(false) ) {
        download.getResult().appendText(paramString);
    }
}
