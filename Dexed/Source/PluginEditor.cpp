...
{
    // Modified from Dexed
    setSize(WINDOW_SIZE_X, (ownerFilter->showKeyboard ? WINDOW_SIZE_Y + 50 : 581 + 50));
    ...
    mMatch.onClick = [&]() { processor->matchFile(); };
    addAndMakeVisible(mMatch);
    
    mDownload.onClick = [&]() { processor->downloadFile(); };
    addAndMakeVisible(mDownload);
    
    mPing.onClick = [&]() { ping(); };
    addAndMakeVisible(mPing);
    
    addAndMakeVisible (addressTextBox);
    addressTextBox.setTextToShowWhenEmpty ("Server Address", Colours::white);
...
//    mLoad.setBounds(220, 685, 60, 30);
    mMatch.setBounds(50, 685, 60, 30);
    mDownload.setBounds(125, 685, 90, 30);
    mPing.setBounds(645, 685, 55, 30);
    addressTextBox.setBounds (230, 687.5, 400, 25);
    resultsBox.setBounds (715, 687.5, 100, 25);
}
...
void DexedAudioProcessorEditor::ping()
{
    File fakeFile(processor->dexedCartDir);
    fakeFile.appendText("Ping");
    
    URL url = URL(addressTextBox.getText()).withFileToUpload(".test", fakeFile, "test");
    
    MemoryBlock block;
    if (url.readEntireBinaryStream (block, true))
    {
        resultsBox.setText("Success");
        processor->serverURL = addressTextBox.getText();
    } else {
        resultsBox.setText("Error");
    }
